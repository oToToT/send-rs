...
are served from `STATIC_DIR` (`./static` by default).
...
| `STATIC_DIR` | `./static` | Bundled browser asset directory |
...
The browser compatibility client is vendored as [`static/app.js`](static/app.js)
with its English resource in [`static/locale.en.js`](static/locale.en.js). These
two compiled files remain until the browser cryptography client is rewritten as
maintainable first-party source. They are served locally and are never
downloaded or built from another checkout. Server-rendered HTML is generated by
